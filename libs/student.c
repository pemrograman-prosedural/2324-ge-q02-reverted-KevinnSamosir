...
int find_id(char *id, int zstd, struct student_t *mhs)
{
  int find_id = -1;
  for (int m = 0; m < zstd; m++)
  {
    if (strcmp(id, mhs[m].id) == 0)
    {
      find_id = m;
    }
  }
...
void assign_student(struct student_t *student, struct dorm_t *dorm, char *id, char *ldorm, int size_student, int size_dorm)
{
    int i;
    int temp_dorm = 0;
...
void move_student(struct student_t *student, struct dorm_t *dorm, char *id, char *ldorm, int size_student, int size_dorm)
{
    int i;
    int dorm_temp = 0;
    int student_temp = 0;
